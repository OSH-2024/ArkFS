        ...
        messages=[
            {
                "role": "user",
                "content": (
                    f'"{prompt}",对于这句话请提取他的“时间(昨天、前天或其他)”“文件类型(仅包含image或者txt)”“有关文件内容的一个名词(阳光、草地或人名等其他名词)”，如果有缺失的信息，用NULL表示。现在你有“增、删、改、查”四种文件功能(对于任务调度你只能返回“增、删、改、查”这四个字的组合作为任务序列)，请你给出这句话对应的任务调度序列，如“增删”这样的序列，如果不涉及具体动作，只查即可，但如果有“移动”“转移”“放置”之类的要求，你就需要添加增删改查的其他功能。按照顺序，以[[时间],[文件类型],[内容名词],[调度序列]]的格式返回给我，其中内容名词翻译成英文。'
                )
            }
        ]
    )
