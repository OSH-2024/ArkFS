...
def search_files(query, modified_time_start=None, modified_time_end=None, k=5):
    # 使用 CLIP 对查询进行编码
    ...
    for path, score in image_results:
        ...
        if info:
            ...
            if (not modified_time_start or modified_time >= datetime.fromisoformat(modified_time_start)) and \
               (not modified_time_end or modified_time <= datetime.fromisoformat(modified_time_end)) and (score > 0.19):
...
def search(modified_time, content):
    #ISO 格式，例如 2024-07-01T23:59:59
    modified_time_start = modified_time[0] or None
    modified_time_end = modified_time[1] or None
    image_results, text_results = search_files(content, modified_time_start, modified_time_end, k=15)
